...
t.test('getAverageReviewsPerCard', t => {
  const getAverageReviewsPerCard =
    require('../getAverageReviewsPerCard.js');

  const result1 = getAverageReviewsPerCard.call(setup1());
  t.equal(result1, 10, 'no revlog entries');

  const result2 = getAverageReviewsPerCard.call(setup2());
  t.equal(result2, 2, 'average during first day of study');

  const result3 = getAverageReviewsPerCard.call(setup3());
  t.equal(result3, 3, 'average after first day of study');

  const result4 = getAverageReviewsPerCard.call(setup3(), 1);
  t.equal(result4, 4, 'reviews older than days are ignored');
  t.end();
});
