...
t.test('getAverageStudyTime', t => {
  ...
  t.equal(result2.toFixed(2), '75.00', 'average with some logs');

  const result3 = getAverageStudyTimePerDay.call(setup2(), 1);
  t.equal(result3, 80, 'reviews older than days are ignored');

  const result4 = getAverageStudyTimePerDay.call(setup3(), 1);
  t.equal(result4, 80, 'during first day');
  t.end();
});
