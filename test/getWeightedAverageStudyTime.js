...
t.test('getAverageStudyTime', t => {
  const getWeightedAverageStudyTime =
    require('../getWeightedAverageStudyTime.js');

  const result1 = getWeightedAverageStudyTime.call(setup1());
  t.equal(result1, 0, '0 if no revlog entries');

  const result2 = getWeightedAverageStudyTime.call(setup2());
  t.equal(result2, 39.743589743589745, 'average with some logs');

  const result3 = getWeightedAverageStudyTime.call(setup3());
  t.equal(result3, 39.743589743589745, 'study gaps are ignored');

  const result4 = getWeightedAverageStudyTime.call(setup3(), 1);
  t.equal(result4, 50, 'reviews older than 1 study days are ignored');
  t.end();
});
