...
t.test('getCountNewCardsToday', t => {
  const getCountNewCardsToday = require('../getCountNewCardsToday.js');

  const r1 = getCountNewCardsToday.call(setup1());
  t.equal(r1, 0, 'No new cards');

  const r2 = getCountNewCardsToday.call(setup2());
  t.equal(r2, 7, 'Three new cards');
  t.end();
});
