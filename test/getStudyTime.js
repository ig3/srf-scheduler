...
t.test('getStudyTime', t => {
  const getStudyTime = require('../getStudyTime.js');

  const r1 = getStudyTime.call(setup1());
  t.equal(r1, 0, 'No study');

  const r2 = getStudyTime.call(setup2());
  t.equal(r2, 70, '7 cards studied');

  const r3 = getStudyTime.call(setup2(), 25);
  t.equal(r3, 110, '11 cards studied');
  t.end();
});
